...
export function useDesktop() {
  const { loading, setLoading, setBanner } = useAppContext()
  ...
  async function handleCopyDesktopDiagnostics(diagnosticsPayload: object) {
  ...
  function syncDesktopWindowTitle(title: string) {
    if (!desktopShell) {
      return
    }
