...
import type { ChatDeviceSession, ChatMessage, ChatSummary, LinkMetadata, RecipientDevice } from '../lib/api'
import {
  createMessage,
  deleteMessage,
...
import { encryptMessageWithSessions } from '../lib/chat-session-vault'
...
import { decryptAttachmentFile } from '../lib/attachment-vault'
import { mergeMessageThread, projectMessage } from '../utils/message-helpers'
import { extractFirstHttpUrl } from '../utils/format'
import { syncChatSummary } from '../utils/chat-helpers'
import { canUseChatSessions, shouldQueueOutboxSendFailure, isOutboxDuplicateClientIdError } from '../utils/crypto-helpers'
...
export function useMessages(params: UseMessagesParams) {
  ...
  // Message items ref sync
  useEffect(() => {
    messageItemsRef.current = messageItems
  }, [messageItems])

  // Attachment playback urls ref sync
  useEffect(() => {
  ...
  async function sendDraftMessage() {
    ...
    const editingTargetMessage =
      activeEditingMessageId
        ? messageItems.find((message) => message.id === activeEditingMessageId) ?? null
        : null
  ...
  // Message load on active chat change
  useEffect(() => {
...
// Intentional no-op to keep the same effect shape for the chatSessions dependency
// in the message load effect without importing the setter (chatSessions are reset
// from the chatSessions hook).
function setChatSessions_noop() {
  // Session reset is handled by useChatSessions hook.
}
